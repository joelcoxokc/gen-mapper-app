  ...
  var Documents = exports.Documents = (_dec = (0, _aureliaFramework.inject)(_documentsApi.DocumentsApi), _dec(_class = function () {
    ...
    function Documents(documentsApi) {
      ...
      this.api.getAll().then(function (docs) {
        ...
        return _this.requested = true;
      });
    }

    Documents.prototype.delete = function _delete(doc, $event) {
      var _this2 = this;

      $event.stopPropagation();
      this.docs.splice(this.docs.indexOf(doc), 1);
      this.api.delete(doc.id).then(function () {
        if (doc === _this2._current) _this2.changeCurrent(_this2.docs[0]);
      });
    };

    Documents.prototype.create = function create() {
      var _this3 = this;
      ...
      this.api.create({
        ...
        return _this3.docs.unshift(doc);
      }).then(function () {
        return _this3.changeCurrent(_this3.docs[0]);
      });
    };
...
define('text!views/tool/tool.html', ['module'], function(module) { module.exports = "<template><div class=\"tool-sidebar\" if.bind=\"canPersist\"><div class=\"tool-sidebar__nav\"><div class=\"tool-sidebar__logo\">GenMapper</div></div><div class=\"tool-sidebar__actions\"><button class=\"tool-sidebar__action\"><icon name=\"paperclip\"></icon></button> <button click.trigger=\"documents.create()\" class=\"tool-sidebar__action\"><icon name=\"add\"></icon></button></div><div class=\"tool-sidebar__placeholder\" if.bind=\"documents.empty\"><span>You don't have any documents! Try importing or creating one using the buttons above</span></div><div class=\"tool-documents\"><div class=\"tool-documents__document ${documents._current.id === doc.id ? 'tool-documents__document--active' : ''}\" repeat.for=\"doc of documents.docs\" click.trigger=\"documents.changeCurrent(doc)\"><div class=\"tool-documents__meta\"><div class=\"tool-documents__title\">${doc.title}</div><div class=\"tool-documents__description\">${doc.content}</div></div><div class=\"tool-documents__actions\"><icon click.trigger=\"documents.delete(doc, $event)\" class=\"tool-documents__icon\" name=\"delete\"></icon></div></div></div></div><div class=\"tool-main\"><div class=\"tool-nav\" if.bind=\"canPersist\"><input class=\"tool-nav__title\" placeholder=\"Document Name\" value.bind=\"documents.current.title\"><div class=\"tool-nav__actions\"><button class=\"tool-nav__save\" disabled.bind=\"!documents.current.dirty\" click.trigger=\"documents.saveCurrent()\">Save</button></div></div><div class=\"tool-view\"><require from=\"components/texteditor/texteditor\"></require><texteditor></texteditor></div></div></template>"; });
